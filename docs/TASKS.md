...
Task NRQ-004 (Frozen Bundle Compatibility Reconstruction & Depth-2 Control Reproduction) is complete: `STOP_NRQ003_BLOCKED` ([ADR-0164](DECISIONS_PHASE_C.md#adr-0164-nrq-004-frozen-bundle-compatibility-reconstruction--depth-2-control-reproduction-fails-on-bundle-loss-stop_nrq003_blocked)). Audited git history at commit `c3f291f`, run manifests, and checkpoint hashes. Reconstructed native Phase A.1 36-token vocabulary schema (`SharedCoreTokens(num_operations=10, arg_span=10)`) into a non-destructive bundle namespace (`runs/nrq004_reconstructed_bundles/`). Resolved schema drift for seeds 1–4, achieving near-ceiling control reproduction (mean recovered EM = 99.50%, functional agreement = 99.96%, passing 6/6), decisively refuting intrinsic model inadequacy. Confirmed Seed 0 failure is exclusively caused by bundle loss (checkpoint overwrite on 2026-09-13). Enforced fail-closed stop gate (`STOP_NRQ003_BLOCKED`): because all 5 seeds are required for NRQ-003 resumption and unauthorized retraining is prohibited, execution halts without proceeding to training or depth-3 search. See the [review document](research/FROZEN_BUNDLE_COMPATIBILITY_RECONSTRUCTION_NRQ004.md).

Task NRQ-005 (Missing-Artifact-Robust Exact-Depth-3 Irreducible Composition Benchmark) is complete: `EMPIRICALLY_SUPPORT_CLOSURE_DEPTH_LE_3` ([ADR-0165](DECISIONS_PHASE_C.md#adr-0165-nrq-005-missing-artifact-robust-exact-depth-3-irreducible-composition-benchmark-supports-depth--3-closure-empirically_support_closure_depth_le_3)). Standalone replacement protocol using integrity-verified seeds 1–4 from `runs/nrq004_reconstructed_bundles/` (Seed 0 excluded for historical artifact loss). Verified prior to model evaluation via symbolic probe that the exact-depth-3 panel possesses 0 functional equivalents across all 72 depth <= 2 candidates. Oracle execution confirmed ceiling substrate capability (mean Oracle EM = 99.05%, exceeding the >= 0.85 floor). Evaluated exhaustive lawful search over all 584 candidates of depth <= 3 against heuristic beam search and controls. Exhaustive search achieved 99.04% mean test functional EM (>= 0.95 across every tested bundle seed). Beam search sensitivity confirmed that depth-3 search failure at width 16 (EM = 36.60%) is a beam-budget truncation artifact, climbing monotonically to 100.0% EM at width 64. Because the strongest lawful deterministic baseline reaches near-ceiling accuracy across all bundles, the depth-3 composition space remains tractable, providing empirical support for ADR-0162's Bounded-Resource Corollary restricted to depth <= 3. See the [review document](research/EXACT_DEPTH3_COMPOSITION_BENCHMARK_NRQ005.md).

Task NRQ-006 (Argument-Closed Deterministic Full-Registry Depth-3 Closure Audit) is complete: `ADR0165_QUALIFIED_BY_FULL_REGISTRY_AUDIT` (`QUALIFIED`) ([ADR-0166](DECISIONS_PHASE_C.md#adr-0166-nrq-006-argument-closed-deterministic-full-registry-depth-3-audit-qualifies-adr-0165-closure-adr0165_qualified_by_full_registry_audit)). Audited all 512 candidate exact-depth-3 recipes over the canonical 8-primitive registry against all 72 depth <= 2 candidates under a pre-fixed finite lawful argument grammar. All dataset generation seeds derived from SHA-256 digests of recipe strings, ensuring bitwise cross-process reproducibility verified across independent processes (dataset manifest hash: `eeee5ac21698a6567e697991f0f97616c0c7963c72635bf922621803f9284d45`, metric deltas = 0.00e+00). Partitioned recipes into 42 invalid, 370 reducible, and 100 irreducible recipes spanning 60 canonical equivalence classes. Confirmed depth <= 3 closure on all length-adequate classes. Discovered sub-curriculum intermediate length contraction under SELECT ($L < 6$); formally qualified ADR-0165's unconditional closure claim and preserved all failure classes in `runs/nrq006_depth3_audit/`. See the [review document](research/ARGUMENT_CLOSED_DEPTH3_CLOSURE_AUDIT_NRQ006.md). Re-aggregated and formally amended by Task NRQ-007 ([ADR-0167](DECISIONS_PHASE_C.md#adr-0167-nrq-007-stepwise-causal-attribution-disentangles-depth-3-failure-mechanisms-adr0166_qualified_by_stepwise_causal_attribution)).

Task NRQ-007 (Stepwise Causal Attribution of NRQ-006 Composition Failures) is complete: `ADR0166_QUALIFIED_BY_STEPWISE_CAUSAL_ATTRIBUTION` (`QUALIFIED`) ([ADR-0167](DECISIONS_PHASE_C.md#adr-0167-nrq-007-stepwise-causal-attribution-disentangles-depth-3-failure-mechanisms-adr0166_qualified_by_stepwise_causal_attribution)). Re-aggregated all 1200 benchmark cells from NRQ-006, reconciling historical counts and enforcing the non-averaging all-cell gate (19/60 classes pass all 20 cells, 558/1200 cells pass both thresholds). Executed 3-arm stepwise causal intervention across all 35 failure recipes ($700$ cells) comparing (A) continuous execution, (B) diagnostic reset with ground truth intermediate re-encoding, (C) standalone length-matched controls, and parameterized controls (Correct, Wrong-argument, None, Wrong-family). Programmatically attributed failure mechanisms to short-sequence capacity deficit in `SORT` on $L < 6$ (7 classes, 330 cells), argument handling on $L=1$ intermediates (4 classes, 250 cells), and cross-bundle parameter divergence (24 classes, e.g. `SHIFT` generalization on $L < 6$ and Bundle 4 chaining degradation). Formally amended ADR-0166 to `QUALIFIED` and reaffirmed `PROGRAM_LINE_CLOSURE_CONFIRMED`. See the [review document](research/STEPWISE_CAUSAL_ATTRIBUTION_NRQ007.md).

Task NRQ-008 (Replication and Support-Budget Sensitivity of the Sole APC-over-Exhaustive Cell) is complete: `FINITE_SAMPLE_SUPPORT_MISSELECTION` ([ADR-0168](DECISIONS_PHASE_C.md#adr-0168-nrq-008-replication-and-support-budget-sensitivity-audit-closes-solitary-anomaly-cell-as-finite-sample-support-mis-selection-finite_sample_support_misselection)). Evaluated the solitary cell where APC appeared to outperform exhaustive search (`NEGATE->REVERSE->SHIFT` on Bundle 4, Seed 101) across intact bundles 1–4, 20 unused data seeds (201–220), support budgets $N \in \{32, 64, 128, 256\}$, and 1024 disjoint held-out inputs per cell (320 cells, 327,680 total eval queries). Demonstrated across two independent processes ($\Delta = 0.00\times 10^0$) that element negation and reversal commute ($\text{Negate}(\text{Reverse}(x)) \equiv \text{Reverse}(\text{Negate}(x))$), exhaustive search achieves parity with oracle execution across all budgets ($\Delta \le 0.0010$, $\Delta = 0.0007$ at $N=256$), zero bundles satisfy the falsification criteria, Bundle 4's lower performance ($0.9129$ vs $0.9147$) is an intrinsic substrate capacity limit rather than a search failure, and conclusively closed the solitary anomaly as a finite-sample support mis-selection artifact under small sample budget. Reaffirmed ADR-0162's Bounded-Resource Corollary and confirmed `PROGRAM_LINE_CLOSURE_CONFIRMED`. See the [review document](research/REPLICATION_AND_SUPPORT_BUDGET_SENSITIVITY_NRQ008.md). **No task is queued.**
