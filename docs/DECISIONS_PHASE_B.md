...
Part of the split `docs/DECISIONS.md` architecture decision log (ADR-0074 through ADR-0081). See `docs/DECISIONS.md` for the full index across all phases.

This file now covers Phase B's own STOP GATEs (B1/B2), the Hard-Negative-Repair sub-effort (`B-C005D`/`R1`/`R2`/`G`), and the Second Diagnostic phase (`B-C005D2`). Two branches that grew out of this phase's `B-C005R3-010` G4 finding continue in their own split files: the repair branch in `docs/DECISIONS_PHASE_B_B2_POST_D2_REPAIR.md` (ADR-0082 onward), and its own Model Bundle Recovery sub-branch in `docs/DECISIONS_PHASE_B_B2_MODEL_BUNDLE_RECOVERY.md` (ADR-0092 onward, active).
