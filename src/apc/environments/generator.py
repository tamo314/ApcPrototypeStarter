...
Novel-*operation* tasks (Task 009) are out of scope here; the extension
point is `operation_names`, which callers can widen with operations
registered via `apc.environments.operations.register_operation`.
"""
...
class Example:
    ...
    category: str  # "known" | "novel_composition"
    split: str
    vocab_size: int
...
class TaskGenerator:
    ...
    def _composition_pool(self, split: str) -> tuple[tuple[str, ...], ...]:
        if split in KNOWN_SPLITS:
            return self.composition_space.known_compositions
        if split == NOVEL_COMPOSITION_SPLIT:
            return self.composition_space.novel_compositions
        raise ValueError(f"Unknown split: {split!r}")

    def generate(self, n: int, split: str) -> list[Example]:
        """Generate `n` examples for `split`, deterministic given `seed`."""
        pool = self._composition_pool(split)
        category = "known" if split in KNOWN_SPLITS else "novel_composition"
        rng = random.Random(_derive_seed(self.seed, f"generate:{split}"))

        examples: list[Example] = []
        for _ in range(n):
            operation_sequence = pool[rng.randrange(len(pool))]
            lengths = self.composition_space.valid_lengths[operation_sequence]
