...
手先並進6・指2・管理2・回転6・台車4の20候補を実装した。探索seed 1300〜1302の
手設計selectorは最終成功2/3、初期MLPは0/3。相対特徴とlearner状態の教師ラベルを追加し、
16候補の学習selectorは別の未使用seed 1700〜1704で最終成功3/5（教師4/5）だった。
統合モデルを次の未使用1800〜1804で評価すると0/5（教師1/5）であり、条件間で安定した
汎化はまだ示していない。台車4候補は同一sceneの手動列で中断・指幅保持を実行確認したが、
20候補を選ぶモデルは未学習。各run、費用、失敗は [実験メモ](docs/ITERATION_LOG.md) に記録した。

**次段階の診断:** 学習selectorのseed 1802での姿勢誤差を特定したが、再ラベル追加・
抽出比率変更・相関stepの間引きでは単独成功に至らなかった。手設計のpitch補助は把持を
改善したが、学習済み能力には数えない。20候補では机との接触を検出した台車経路検査を修正し、
同一sceneで前進→腕操作→必要なら後退回復を実行。未使用seed 2000〜2004の最終成功は
回復付き3/5、35 step待機対照4/5で、現行PickCubeの初期配置では台車前進の利点は未確認。
詳細は [計画書の継続結果](docs/PRIMITIVE_DECISION_PLAN.md) 第12節を参照する。

**遠方開始のN候補:** 同じ机・物体分布のままFetchだけを20 cm後方から開始する条件を追加。
seed1901〜1903では台車なし教師の最終成功0/3に対し、20候補の手設計台車→腕列は3/3。
状態ベースの列は別seed2301〜2303で2/3、2401〜2403で最終2/3だった。
20候補MLPは台車移動と腕への切替を学んだが、DAgger追加後も新seed2401〜2403で0/3。
既存の局所制御器で成功例があるため、今は腕局面のselector誤りを調べる。
...
**次の診断（同日）:** 失敗ラベル追加、把持状態で分けた木、回転介入による収集を比較した。
未使用2901〜2905の最終成功は前回モデル2/5、追加学習モデル2/5、教師3/5で、改善は未確認。
遠い物体への拒否にはIK不成立・関節範囲違反が含まれ、机経路だけが原因ではなかった。
後退、軸順序変更、水平姿勢も探索3条件で成功を増やさず、水平姿勢では接触を検出した。
[計画書第15節](docs/PRIMITIVE_DECISION_PLAN.md) に失敗と次の切り分けを記録。
下の再実行例は比較基準の149ノードモデルを使う。

**到達範囲とIKの診断（同日）:** 失敗状態を再現し、目標姿勢・接近高さ・IK初期値を比較。
元の失敗2例では別初期値でも前進目標の解が得られなかった。一方、2 cm追加前進・水平姿勢の
下降では、同じ目標がリセット時の腕姿勢を初期値にすると解ける例を確認した。
`--ik-reset-seed`で通常のIK失敗時だけ再試行できる（既定は無効）。実行で3 stepを回復したが、
探索3条件の最終成功は0/3で、タスク性能の改善は未確認。固定0/5/15度と接近位置の比較、
接触、費用は [計画書第16節](docs/PRIMITIVE_DECISION_PLAN.md) と実験メモに記録した。

**移動量・下降姿勢・把持高さの比較（同日）:** 拒否時だけ移動量を半減する実行オプションと、
下降時の姿勢切替を追加した。把持高さ25 mmで既知の難しい配置2803を初めて完了し、同じ
1,200 step比較で高さ20 mmの0/3に対し25 mmは1/3、接触0だった。ただし未使用3001〜3003では
今回設定0/3・従来教師3/3。把持後に落としており、既定は従来の設定を維持する。
学習済み能力の改善ではなく手設計診断である。詳細は [第17節](docs/PRIMITIVE_DECISION_PLAN.md)。
...
で更新した。上空姿勢切替（pre-rotate）、閉指待機付き把持回復、把持高さ最適化により、
難関seed 2801〜2803および滑り落ちseed 3001〜3003で3/3・3/3（机接触0 N）、未知seed 3101〜3105で
初回4/5・最終3/5（接触0 N）を達成し、安定した手設計教師ベースラインを確立した。
現在の次の一点は、この安定データを用いた学習selectorの改善、またはtemporary管理サイクルへの展開である。
