...
def main():
    ...
    parser.add_argument("--selector", choices=["manual", "pick_place", "mlp"], default="manual")
    ...
    config = RunConfig(env_id="APC-FetchPickCube-v1", robot_uids="fetch", policy="external",
                       ...
                       task_label=args.selector + ("_fetch16_primitives" if args.rotations else "_fetch10_primitives"))
    ...
    def factory(env, output):
        ...
        if args.selector == "mlp":
            ...
            selector = PickPlaceSelector(use_rotation=args.rotations) if args.selector == "pick_place" else None
        return PrimitivePolicy(env, output, allow_rotation=args.rotations,
