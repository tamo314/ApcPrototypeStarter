...
**今後の設計:** [小さな身体操作と毎step判断](PRIMITIVE_DECISION_PLAN.md) に、
プリミティブ、保持目標、判断入力、全身指令の生成、記録と実装順序を定義した。
初版10候補・拡張20候補を直接選択する方式は未実装。本書の実装済みの説明は維持し、
後半の大きなスキルと終了時selector復帰の案は履歴として残す。
...
次に実装する経路は [後続設計](PRIMITIVE_DECISION_PLAN.md) 第4・7節を参照する。
以下の終了待ちを中心とする構成から、毎step判断と保持目標の更新へ変更した。
...
大きなスキル向けの旧案。初版の実装契約は後続設計のプリミティブID・保持目標・実行器を用いる。
