...
### 2026-09-24 — 把持分割・回転介入・IK拒否の診断

- 基点 `775ed86`。専用WSL Python3.12/CPU、ManiSkill3.0.1、Fetch、`APC-FetchPickCubeFar-v1`、状態観測、20 Hz制御、900 step上限、初回成功後20 step観測。最初に既存失敗ラベルを再利用して1 fit＋探索5 episodeとし、結果を見て把持状態による分割、局所回復の各3 episode診断、介入収集、未使用5 seedでの対照へ広げた。全出力は `experiments/maniskill/runs/` 以下の新しいディレクトリ。旧run・checkpointは変更していない。
- 学習: `primitive-next-dagger-train-20260924-a/` は前回149ノードの全sourceに前回tree2701/2801の問い合わせ8,250 sampleを加え、31,371 sample・201ノード（訓練accuracy99.923%）。`primitive-next-partition-train-20260924-a/` は同じデータで観測`grasped`を木の根に指定、181ノード・99.608%。この分割だけは手設計の構造上の事前設定で、両状態を含む訓練データを必要とする。teacher stage/実行時teacherの補助は使わず、下位の分岐と操作は学習する。
- 回転介入: `RotationProbeSelector` は450/520/590/660/730 stepでID12/13を交互に強制し、それ以外は固定した把持分割モデルを使う。`tree_probe`のmanifestは`learned=false, exploration=true`、stepには`probe_applied`と`probe_raw_id`を追加。2804〜2806の15回の実介入、2,599実訪問stateへ従来教師を問い合わせた。教師/実行ID不一致は各10/169/31件。`primitive-next-probe-train-20260924-a/` はこの収集だけを追加して33,970 sample、199ノード・訓練accuracy99.635%。これら3 fitは全て深さ12・最小葉2・sqrt逆頻度重み・v5特徴・標準偏差下限.01、勾配更新0。sourceのパス/step SHA256/ラベル数/構成は各training.jsonに保存。

| run（`runs/` 下） | seed / episode / step | 初回・最終成功 | 役割 |
|---|---:|---:|---|
| `primitive-next-dagger2801-20260924-a` | 2801〜2805 / 5 / 4,250 | 1/5・1/5 | 失敗ラベルを追加。前回同seed2/5より低下 |
| `primitive-next-partition2801-20260924-a` | 同 / 5 / 4,283 | 1/5・1/5 | 同じデータ、把持状態で分割 |
| `primitive-next-recover2801-20260924-a` | 2801〜2803 / 3 / 2,700 | 0/3・0/3 | 拒否5回で既存ID17の後退を1回、35 step待ち |
| `primitive-next-axis2801-20260924-a` | 同 / 3 / 2,700 | 0/3・0/3 | 並進拒否後に目標へ近づく別の軸を先に動かす |
| `primitive-next-pitch0-2801-20260924-a` | 同 / 3 / 2,700 | 0/3・0/3 | 教師pitch15度→0度。前腕接触あり |
| `primitive-next-probe2804-20260924-a` | 2804〜2806 / 3 / 2,599 | 1/3・1/3 | 回転介入ありの収集。学習器単独成績ではない |
| `primitive-next-probe-learned2801-20260924-a` | 2801〜2805 / 5 / 4,283 | 1/5・1/5 | 介入ラベルを学習後、介入なしの探索比較 |
| `primitive-next-probe-learned2901-20260924-a` | 2901〜2905 / 5 / 3,984 | 3/5・2/5 | 固定した今回モデルの未使用評価 |
| `primitive-next-before2901-20260924-a` | 同 / 5 / 3,651 | 4/5・2/5 | 前回149ノードの同seed対照 |
| `primitive-next-teacher2901-20260924-a` | 同 / 5 / 3,647 | 4/5・3/5 | 手設計教師の同seed対照 |

- 拒否の分類を訂正: 過去の「腕経路拒否」の多くは実装のreason2全体の件数であり、IK不成立・関節範囲・机経路のいずれかである。2802/2803の旧教師の最初の失敗はstep469/452の前方ID0、`ik_success=false, ik_within_limits=false, ik_table_clear=true`だった。後退runはstep474/457でID17を安全検査が許可し、台車X約.201→.181 mへ実移動したが失敗。後退後を含む腕拒否396/413件は全てIK失敗を含み、机経路flagの失敗は0件。手設計回復は一度だけで、後退後に初期前進列へ戻らないよう記憶する。
- 軸順序/姿勢: 拒否された並進IDと次の教師IDが同じなら、目標誤差が3 mm以上残る別軸の最大成分を先に選ぶ診断を追加。2802/2803の腕拒否は旧教師431/448→251/59件へ減少したが、最終手先X約.063 m、物体X約.095 mで把持せず。単純な軸順序変更だけでは解決しない。`--selector base_ready_pick --pitch-deg 0`は教師の固定姿勢だけを変える診断で、学習へは未使用。2802/2803はIK失敗310/328件。2801は机経路失敗323件に加え、step581〜587の7件で前腕と机の接触、最大2.9986 N。0度姿勢は採用改善としない。
- 残る判断誤り: 2804では把持中、教師の上昇ID4をモデルが回転ID12へ置換した。最初の当該分岐はfeature索引66（grasped）→32（角度追従誤差）→17（持上げ量）→5/0/61/3/62/1という位置・幾何特徴へ進んでいた。回転介入後も探索成功数は変化しなかった。未使用2901のモデルは2904/2905、旧モデルは2903/2904で最終成功。2902は初回成功後の観測完了時に失敗し、初回と最終を分けて数えた。新旧とも2/5であり、今回の追加学習の優位は未確認。
- 独立性/再現: 2701/2801は今回から学習にも使用し、探索扱い。2806は介入収集で初使用し、そのまま学習へ入れた。2901〜2905は学習sourceと非重複。結果を見てから再学習なし。最終source seedは1901〜1903、2301〜2303、2401〜2403、2501〜2503、2601〜2605、2701〜2705、2801〜2806。既存IK追従器・台車経路検査は変更していない。新プリミティブ本体やtemporary獲得ではない。
- 実績/計測: **10 runner run / 42 episode / 34,797環境step**、episode wall合計**474.3秒**。**CART3 fit、勾配更新0**。過去収集データの再利用は環境stepへ重複加算せず、今回の介入runは含む。全runner completed、例外なし。水平姿勢診断以外の9 runは全記録stepの接触0 N。新しい `scripts/analyze_primitive_runs.py` と `primitive-next-report-20260924-a/report.json` にsource SHA256、episode別のIK/関節/机経路失敗（重複可）、接触、問い合わせ数/混同行列を保存。未queryなら不一致はnull、接触の未計測と0も区別する。
- 検証: 既存1本の実環境統合テストを拡張。遠方教師を550 step×2 episodeへ延ばして把持前後の両データを含め、把持分割の学習→保存→ロード、queryの行動不変、後退530 step、軸変更490 step、明示介入30 stepとログを確認。`primitive-next-feature-tests-20260924-a/` は **1 passed in84.75s**、3,540テストstepはrunner総計外。成功数は合否に使わない。新しい集計器は今回の全実runで実行し、Python compileと差分確認も通過。既知のVulkan/glvnd/Pinocchio/NumPy警告のみ。
- 次: 今回は成功率の改善を主張せず、比較基準の前回149ノードを保持する。到達しない目標のIK解と姿勢・関節制約を局所的に比較し、目標表現と局所制御不足を切り分ける。把持後の状態では姿勢と位置を独立に変える収集の範囲が不足している可能性が残る。単純な追加ラベル、把持分割、15回の回転介入だけで解決したとは言えない。

### 2026-09-24 — 同一状態の到達範囲と別初期値IK

- 基点 `f62e41e`。既存WSL Python3.12/CPU、Fetch遠方開始、20 Hz制御。最初は既知の失敗3条件を500 step再現する予算とし、診断結果に応じて追加前進・姿勢・IK初期値の各3 episode比較へ拡張した。全て探索seed2801〜2803。学習器の更新・独立評価・新ID追加は行っていない。以下のrunは `runs/` 以下で、旧runは保持。
- 実装: `scripts/probe_primitive_reachability.py` は最初のreason2で現在関節、root、要求目標、関節範囲、初期値別候補解をJSON保存。現在/リセット腕×胴体自由/.1/.2/.3/.386 mの10初期値、元目標/半分・ゼロ移動/姿勢変更/物体上の高さ変更を比較。`feasible`はIK成功・関節範囲・現状態からの有限サンプル机経路を通過した意味で、物理実行成功ではない。物理状態を変えず、送信actionの再現一致は旧教師との1,500/960 step、追加前進0度対照との1,800 stepの全件で確認。保存候補は1,000件、source SHA256と比較は `primitive-reach-report-20260924-a/audit.py`・`audit.json`。

| run（全て末尾 `-20260924-…`） | seed / episode / step | 初回・最終成功 | 測定 |
|---|---:|---:|---|
| `primitive-reach-probe2801-20260924-b` | 2801〜2803 / 3 / 1,500 | 0/3・0/3 | 500 step診断。2801はこの上限内で拒否なし。2802/2803の最初の拒否を保存 |
| `primitive-reach-probe2802-20260924-c` | 2802〜2803 / 2 / 960 | 0/2・0/2 | 480 step。物体上の高さ/姿勢目標まで比較を追加 |
| `primitive-reach-forward2801-20260924-a` | 2801〜2803 / 3 / 2,700 | 0/3・0/3 | 切替X .215 m、pitch15度。拒否414/584/584件は全て机経路flag |
| `primitive-reach-forward-pitch0-2801-20260924-a` | 同 / 3 / 2,700 | 0/3・0/3 | 同じ追加前進、pitch0度。2802はIK拒否170、2803は机経路246、接触4 step |
| `primitive-reach-probe-forward2802-20260924-a` | 2802〜2803 / 2 / 1,800 | 0/2・0/2 | 追加前進0度の最初の拒否を再現。action不変なので2803の接触も再現 |
| `primitive-reach-reseed2801-20260924-a` | 2801〜2803 / 3 / 2,700 | 0/3・0/3 | 追加前進0度＋IK初期値再試行。2803で3 step回復、接触0、把持なし |
| `primitive-reach-pitch5-2801-20260924-a` | 同 / 3 / 2,700 | 0/3・0/3 | 追加前進5度、再試行なし。IK/机拒否が残り、2803に接触11 step |

- 元状態: 2802/2803のstep469/452では前方1 cmの目標を10初期値とも解けない。水平姿勢にすると4/6候補が有効。ただし物体上の高さ12/6/3/1.2 cmとpitch0/15/30度では有効解なし。これは有限の初期値・姿勢探索であり、到達不可能の証明ではない。`--base-switch-x-m .215`を追加し、測定X約.221 mで腕へ移る比較を可能にした。台車の既存経路検査は維持。
- 追加前進0度: 2802のstep641では1 cm前進が解けず、5 mm前進は4候補が通過。2803のstep651では同じ下降目標がリセット腕＋胴体.2 mなら通過した。元の制御器には胴体高さの再試行しかなく、この解を発見しない。通常の全試行が失敗した場合だけリセット腕から胴体自由＋4高さを再試行する `--ik-reset-seed` を追加した。要求位置/姿勢は変更せず、sceneもresetせず、実状態からの経路を検査して最小関節距離の有効解を選ぶ。`ik_reset_seed_attempts`/`ik_reset_seed_used`とmanifest設定を保存。既定は無効。
- 再試行の効果/費用: 2801/2802/2803の追加IK計算は1,575/850/1,240回、計3,665回。2803で3 stepのみ回復し、腕拒否246→245、接触4→0 stepだった。最終成功・把持は依然0/3であり、局所回復を能力獲得とみなさない。運動学診断の5/10度に局所解があったので固定5度を実行したが、拒否302/142/245件、全て把持なし。状態に応じた姿勢・経路が必要かを次に調べる。
- 接触: 追加前進0度の2803に4 step/最大0.0608637 N、同設定のprobeでも再現。追加前進5度の2803に11 step/最大2.4769843 N。他の完了runの全記録stepは0 N。有限サンプル検査は動的接触を保証しない。低い力も0へ丸めず記録した。
- 実行障害: 最初の `primitive-reach-probe2801-20260924-a` は非有界関節の±infをstrict JSONへ保存しようとして失敗。500 stepの完了episode＋2番目の469 step、計969実行stepを保持。非有界範囲をnullへ修正し `-b` で再開。IKの非有限解は従来通り例外にする。集計監査の最初の実行は保存field名を`action`と誤認しKeyError、実際の`info.diagnostic.submitted_action`へ修正して再実行した。元runデータは変更していない。
- 実績: **完了7 run/19 episode/15,060環境step、episode wall241.6秒**。失敗runの969を含め**16,029実行step**。別初期値3,665回と保存運動学候補1,000件を環境stepと分ける。学習fit/勾配更新0。短いprobeの0成功を900 step本試行と比較しない。全episode結果、拒否の重複可能分類、接触、IK再試行数は `primitive-reach-report-20260924-b/report.json`。初期化時間・失敗run時間は上記wall合計に含まない。
- 検証: 既存 `test_primitive_feature.py` 1本に実環境からの拒否→候補保存→元actionとの一致、切替位置、別初期値の有効解から送信actionを作る経路を追加。診断追加時 `primitive-reach-feature-tests-20260924-a` は **1 passed in96.42s**（4,255 step）、IK再試行追加後 `-b` は **1 passed in104.93s**（4,930 step）。計9,185テストstepは実験総計外。成功率をテスト合否に使っていない。既知のVulkan/glvnd/Pinocchio/NumPy警告のみ。Python compileと差分検査も実行。
- 次: 物体直前で1 cmは拒否され5 mmは通るケースには、失敗時だけ移動量を縮める比較を行う。机に近い下降には接近時と把持時で姿勢/関節経路を変える候補を小規模探索する。今回の結果だけで既存20 IDでは不可能とも、新しい身体プリミティブが必要とも結論しない。把持後の学習selector誤り、temporary獲得/蒸留/解放は今回未実施で、継続課題として保持。

### 2026-09-24 — 拒否時半減、下降姿勢、把持高さの同条件比較

- 基点 `4bfa702`、既存WSL Python3.12/CPU、Fetch遠方開始、20 Hz制御、初回成功後20 step観測。最初は既知の失敗2801〜2803を900 step×3 episodeとし、拒否の局所回復、下降姿勢、把持高さ、別初期値の併用を順に比較。運搬途中だった設定は高さ20/25 mmの両方を1,200 stepへ延長した。最後に設定を固定して未使用3001〜3003を同じ1,200 stepで従来教師と比較した。全runは `runs/` 以下に保存。
- 実装: `--translation-backoff` は並進ID0〜5が拒否された場合だけ距離半分の同方向・同姿勢目標を1回再試行する。標準1 cm→5 mm、半分の距離が追従許容3 mm以下になる設定は拒否する。元solverの全結果、元/再試行目標、採用flagを保存し、最終reason2と最初の拒否を分ける。IK・関節範囲・机経路検査は維持。継続/回転/台車は半減しない。既定無効。集計器は2回のsolver費用を合算し、IK初期値再試行とも二重計上しない。
- 手設計selector: `--descend-pitch-deg 5 --pitch-deg 0` は水平距離12 mm未満、手先が物体上80 mm以下になったら0→5度に切り替え、以後保持する。切替は測定状態で判定し、毎stepにphaseと目標pitchを保存。`--grasp-height-m` は物体中心からの手先高さを指定する。全て`base_ready_pick`専用の診断設定で、学習モデルやteacher queryで自動的に新教師へ変更する仕組みではない。全試行は台車切替X .215 m（従来教師対照のみ .195 m）。

| run（全て末尾 `-20260924-a`） | seed / episode / step | 初回・最終成功 | 結果 |
|---|---:|---:|---|
| `primitive-adapt-half2801` | 2801〜2803 / 3 / 2,700 | 0/3・0/3 | 前回の水平姿勢＋半減だけ。770再試行で4 step回復、把持なし |
| `primitive-adapt-stage5-2801` | 同 / 3 / 2,700 | 0/3・0/3 | 下降時5度、把持高さ12 mm。365再試行で2 step回復、把持なし |
| `primitive-adapt-height20-2801` | 同 / 3 / 2,700 | 0/3・0/3 | 高さ20 mm。2803を222 step把持するが持上げ拒否 |
| `primitive-adapt-reseed2801` | 同 / 3 / 2,700 | 0/3・0/3 | 高さ20 mm＋IK別初期値。2801の10 step回復、2802/2803の行動は変わらない |
| `primitive-adapt-height25-2801` | 同 / 3 / 2,700 | 0/3・0/3 | 高さ25 mm。2802一時把持6 step、2803把持224 step・最大16.48 cm持上げ、接触0 |
| `primitive-adapt-height25-long2801` | 同 / 3 / 3,441 | 1/3・1/3 | 上限1,200 step。2803を1,041 stepで観測完了、接触0 |
| `primitive-adapt-height20-long2801` | 同 / 3 / 3,600 | 0/3・0/3 | 高さ20 mmの同予算対照。接触/持上げ拒否が残る |
| `primitive-adapt-final3001` | 3001〜3003 / 3 / 3,600 | 0/3・0/3 | 固定した高さ25 mm設定。全3例一時把持後に喪失、腕拒否・机接触0 |
| `primitive-adapt-before3001` | 同 / 3 / 2,110 | 3/3・3/3 | 従来の手設計教師（15度、高さ12 mm、通常移動、X .195 m）、接触0 |

- 半減の再現: 初回半減までの585/641/651 step、計1,877送信actionは前回 `primitive-reach-forward-pitch0-2801-20260924-a` と完全一致。半減のみは腕拒否313/207/247件、接触6/9/9 step・最大7.721/3.329/4.210 N（上腕/手首）。運動学で通る小移動を追加するだけではタスクを改善しない。
- 姿勢/高さ: 5度切替の2801はstep577、下降目標まで7.30 mm残る状態で回転ID12を選び、机経路に拒否された。2802/2803は切替自体は通るが下降/把持が残る。12 mm設定では接触0/7/13 step、最大0/6.637/2.894 N。20 mmでは2803がstep678で把持する直前の676/677に`wrist_roll_link`接触（最大23.524 N）、その後も机経路拒否で持上げできない。2802は接触7 stepで閉指せず。高さ25 mmでは900/1,200 step両runの接触0、2803で保持・運搬・最終成功へ進んだ。2802は最大5.71 mm持上げたが把持を失い、既知条件でも全例成功ではない。
- 別初期値併用: 20 mm設定への追加IK再試行は計6,615回（半減の前後を合算）、2801の10 step回復だけで最終0/3。元runとの送信action一致2,377/2,700、2802/2803は全件一致。回復数と成功数は別に記録する。
...
### 2026-09-24 — クリーン教師データによる20 IDセレクターの再学習と未知seed自律閉ループ汎化評価

- 基点 `b23323b`、WSL Python3.12/CPU、Fetch遠方開始、20 Hz制御、初回成功後20 step観測。
- 課題と実装:
  - 1. **クリーン教師データの統合**: 接触0・拒否0の安定教師データ（難関seed 2801、滑り落ちseed 3001、基準seed 1901、初期未知seed 3101）から、ノイズや破損のない高品質な14エピソード（計11,711環境step）を統合。
  - 2. **自律セレクターの再学習**:
    - CART決定木（v5幾何特徴、sqrt逆頻度サンプリング、把持状態分割）: 10,697サンプルで1 fit、145ノード、検証精度**99.11%**。
    - MLPセレクター: 3,000勾配更新、検証精度84.81%。
  - 3. **自律閉ループ実行評価**: 手設計ルールや教師呼出しを一切排し、学習済みモデル単独（`--selector tree` / `mlp`）で遠方開始タスクを実行。
- 実績表:

| run（全て末尾 `-20260924-a`） | seed / episode / step | 初回・最終成功 | 把持・運搬到達 | 観察 |
|---|---:|---:|---:|---|
| `primitive-clean-tree-eval2801` | 2801〜2803 / 3 / 2,830 | **3/3・2/3** | 3/3 | 9 ep学習器。2801(691st), 2803(974st)完走、全例接触0 N |
| `primitive-clean-mlp-eval2801` | 同 / 3 / 3,600 | 0/3・0/3 | 2/3 | MLP。2802で518st把持・運搬も2803で台車拒否 |
| `primitive-clean-tree-eval3101` | 3101〜3105 / 5 / 5,466 | 1/5・0/5 | 2/5 | 9 ep学習器の未知評価。配置カバレッジ不足判明 |
| `primitive-augmented-tree-eval3201` | 3201〜3205 / 5 / 5,159 | **2/5・2/5** | **4/5 (80%)** | 14 ep学習器（145ノード）。完全新規seedで3201(754st), 3203(805st)完走、4/5把持運搬 |
| `primitive-stable-baseline-eval3201` | 同 / 5 / 4,718 | 3/5・3/5 | 5/5 | 同seed手設計教師対照。3201, 3203, 3205成功 |
...
### 2026-09-26 — A再遭遇・candidate_A再学習・未使用seed評価

- 問い/今回変えた点:
  - A+C銀行で再検出したA型イベント（3058・3063・3065）を同じ獲得器で処理した。
  - 既存の決定行と合わせてcandidate_Aを再学習した（A+C2）。
  - 未使用seed 3066〜3089で評価した。
- 環境step・episode・学習step・wall time等の予算と実績:
  - 320,958 step（再検出6,000、リプレイ16,910、探索＋再生176,291、行列121,757）。
- runパス / 実データの観測 / 失敗した条件:
  - `runs/t34-matrix-20260926-b`：未使用24条件で、初期12、A+C 15、A+C2 12。
  - A+C2は学習条件3058・3063を得たが、3023・3072・3079・3077を失った。
  - `runs/t34-acquire-A2-20260926-a-failed-bankroles` はworker内のBank生成の不具合で0 stepのまま停止（修正済み）。
- 解釈（測定と推測を分ける）:
  - 測定：A（決定4行）・C（決定2行）の小さなCARTが未使用条件で4件の成功を得た。
  - 推測：再学習の劣化は、ルーターが87ノードへ増え、新規ラベル重み50に過適合したためと考える（未検証）。
- 次に変える一点:
  - 新規ラベル重みとルーター深さを下げ、開発リプレイでの保持を確認しながら再学習を比較する。
- 機能テストを実行した場合の結果 / 未実行と理由:
  - コード変更は `acquisition.py` のBank生成1行と `--prior-acquisition` の追加のみ。機能テストは再実行していない（前回 1 passed）。

### 2026-09-26 — T34〜T44（課題列での自動継続獲得・蒸留・最終独立確認）

- 問い/今回変えた点:
  - 計画 `docs/APC_NEXT_PLAN_T34_T44_20260926.md` を作成し、その順に実行した。
  - 課題列runner `run_t34_stream.py`（オンライン検出→獲得→採用判断→S[t,j]）、蒸留 `run_t37_distill.py`、共有基準DAgger `run_t39_shared_dagger.py`、集計 `collect_t34_t44.py` を追加した。
- 環境step・episode・学習step・wall time等の予算と実績:
  - 本段で1,994,500 step（累計3,544,615）。課題列1本あたり約1.5時間。
- runパス / 実データの観測 / 失敗した条件:
  - 詳細は `docs/T34_T44_EVIDENCE.md`。
  - 課題列の変遷：a（有害更新を却下）→ b（先の獲得を喪失）→ c（保持を守るが後続更新が止まる）→ d（S 6→7→7→8、喪失ゼロ）。逆順も最終S=8。
  - T42（3090〜3129）：初期23/40、APC各銀行20〜23/40、共有DAgger 4/40。
- 解釈（測定と推測を分ける）:
  - 測定：閉ループと継続保持は開発規模で機能した。独立条件での成功率改善は再現しなかった。
  - 推測：Candidateの起動判断と、決定行の少なさがボトルネック。
- 次に変える一点:
  - 採用判断に「開発群の別条件での純効果」を加え、誤介入を抑える。次の独立群は3130以降。
- 機能テストを実行した場合の結果 / 未実行と理由:
  - `tests/test_stream_feature.py` が1 passed（469秒）。以降の変更は課題列runnerの保持・リプレイ集合のみで、runで動作を確認した（テストは再実行していない）。

### 2026-09-26 — T45〜T50（誤介入分析・適用可能性ゲート・親委譲拡張・最終独立確認）

- 問い/今回変えた点:
  - 計画 `docs/APC_NEXT_PLAN_T45_T50_20260926.md` を作成し、その順に実行した。
  - 追加：銀行の役割 `gate`（Candidate適用可能性ゲート）と `parent_router`（親委譲）、反実仮想ラベル `run_t47_counterfactual.py`、`make_deferring_bank.py`、`analyze_t46_interventions.py`、集計 `collect_t45_t50.py`。
- commit / config / robot・controller / seed・split:
  - 調整群：3066〜3113、3115〜3129、3300〜3359（123条件）。
  - 最終独立群：3130〜3189（60条件、1回のみ評価）。
  - 監査：前段T42に開発seed 3114が混入していた。除外しても結論は同じ。
- 環境step・episode・学習step・wall time等の予算と実績:
  - 本段で982,612 step（累計4,527,227）。
- runパス / 実データの観測 / 失敗した条件:
  - 詳細は `docs/T45_T50_EVIDENCE.md`。
  - 反実仮想の継続分岐は、154/154点で記録と一致した。
  - v3でCandidateを常時拒否すると、調整群で60/123（初期71）になった。損失の主因は、再学習ルーターが元の判断を置き換えたことだった。
  - 親委譲：調整群68/123。Candidate非起動の条件は、初期とstep数まで一致した。
  - T49：初期26/60、v3 29、親委譲29（得3・失0）、親委譲＋ゲート27。
- 解釈（測定と推測を分ける）:
  - 測定：親委譲は非干渉を構成で保証した。適用可能性ゲートは独立汎化しなかった。Candidate Aの正味効果は、調整群と最終群の合算で0と区別できない（97対97/183）。
  - 推測：Candidateの決定行が少なく（5〜10行）、適用条件が16次元の状態特徴では分離できないことが上限を決めている。
- 次に変える一点:
  - 親委譲を課題列の既定設計にし、採用判断をCandidate起動条件での対比較に絞る。次の独立群は3190以降。
- 機能テストを実行した場合の結果 / 未実行と理由:
