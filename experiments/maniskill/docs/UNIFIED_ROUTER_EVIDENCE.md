...
> [!NOTE] 統合ルーターの実際の実行構成に関する客観的明記
> 現時点の実装（`UnifiedRouterSelector`）は、純粋に決定木のみで全ての判断を完結しているわけではなく、以下のハイブリッド構成となっている：
> 1. **未把持時の回復ガード（GraspRecoveryGuardSelector）の先行実行**: 未把持状態（`not grasped`）では、CART の判定に先立って手設計の状態機械である回復ガードが優先実行される。
> 2. **学習ルーター（19ノード CART）**: キューブ把持後の運搬（Class 2）、配置への移行（Class 3）、および Base 継続（Class 0）の選択を一元判定する。
> 3. **終端配置ラッチ（latched_place）**: 配置が一度開始されると、エピソード完了または reset まで配置モジュールが継続ラッチされる。
> したがって、「全ての適用・引き継ぎ・復帰が単一決定木へ一元化された」とするのは不正確であり、「**学習したルーター＋手設計の安全回復処理＋終端配置ラッチ**」と記述するのが正確である。低位の安全制約や終端ラッチを残すことは工学的に妥当であるが、回復規則自体を学習済み能力として過大に計上しないことが重要である。

1. **ゲート乱立の解消と平坦化**:
   - 多重にネストされた個別セレクターを、19 ノード（4 KB）の決定木ルーターを中心とする統合アーキテクチャ（`UnifiedRouterSelector`）へ整理・平坦化した。
2. **命題 H2（適用可能性）の評価**:
   - 把持後の局所モジュール選択（Base / Transit / Place）が状態観測から自律決定され、タスク間の干渉や副作用を生じさせることなく、新規課題の解決と過去能力の保持が成立することを確認した。
3. **W4（適用条件の学習: T11–T13）の到達度**:
   - T11（運搬ゲートの学習）、T12（引き継ぎ境界・復帰条件の検証）、T13（統合ルーターの学習）の機構検証を物理シミュレータ上で完了した。
