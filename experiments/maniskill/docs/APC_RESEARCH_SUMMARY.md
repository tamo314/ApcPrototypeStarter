...
1. **多段学習モジュール（運搬＋配置）の自律転移の実証（seed 3010, 3013）**:
   - 完全未知配置（seed 3010, 3013）において、手設計の運搬ガードを排し、学習した局所運搬モデル（Temporary MLP / 定着 Candidate CART）と固定配置 Candidate CART の合成によって、**完全未知状態への適応・整定・手放し（20 step維持）が成立**しました。
   - **所要ステップの解釈**: seed 3010 では Candidate CART（758 steps）が Temporary MLP（1059 steps）より短縮されましたが、開発条件 seed 3007 では Candidate CART（927 steps）が Temporary MLP（804 steps）より長くなっています。どちらが短く完了するかは条件によって逆転しており、Candidate への置換後も成功する条件がある一方で、軌跡や所要ステップには変化が生じることを客観的に示しています。
   - **失敗時の状態差**: seed 3012 では Temporary MLP の到達最小距離が 4.55 cm、Candidate CART が 12.80 cm でした。両者とも「配置未到達・失敗」という大分類は一致しても到達した幾何状態は大きく異なっており、同じ成否を蒸留の忠実性と同一視しない方針を維持します。
2. **多方向拡張の範囲の限定性**:
   - データセットへの `+X` ラベル追加により X方向の補正は獲得されましたが、172 サンプル中 133 件は `CONTINUE` であり、`-X` や `-Y` は依然として含まれていません。広い幾何条件への対応が完了したわけではなく、特定の軸方向補正が加わった段階です。
3. **ボトルネックの明確な層別化**:
   - **把持層の制約（seed 3008, 3009）**: Base CART 自体が把持に失敗する（未把持または途中脱落）ケースでは、運搬・配置モジュールは起動すらされません。これは全体タスク成功率を制限する要因が把持層にあることを客観的に示しています。
   - **運搬層の学習モデルと手設計ガードの担当範囲の相違**: 手設計ガードは Base が上昇操作を選びかつ一定の高さ以上の数回のみ介入する限定補正であるのに対し、学習モデルは区間全体（数百 step）をまとめて引き受けています。そのため、学習モデルの失敗には「覚えた判断の不足」と「任せる範囲が広すぎる可能性」の双方が存在します。

---

## 12. 運搬モデル介入範囲のアブレーション診断と回帰検証（2026-09-25 追記）

### 12.1 同一保存状態における判断分岐診断（seed 3007, 3011, 3012）
診断スクリプト（`scripts/diagnose_transit_divergence.py`）により、手設計ガード軌跡から同一の観測状態を入力し、手設計ガード・更新 Temporary MLP・更新 Candidate CART の出力を直接比較しました。

| Seed | 運搬広域区間 | ガード発動回数 | ガード発動時の判断不一致 | 主な判断差と状態幾何 |
|---|---|---|---|---|
| **3007** (開発) | 76 steps | 6 回 | **0 / 6 (100% 一致)** | ガードが選ぶ `+X` (0) と `+Y` (2) を学習モデル双方が完全再現 |
| **3011** (分析) | 107 steps | 2 回 | **2 / 2 (100% 不一致)** | ガードは `-Y` (3) を出力。学習データに `-Y` が無いため MLP は `CONTINUE` (8)、CART は `+Z` (4) を出力 |
| **3012** (分析) | 105 steps | 16 回 | **16 / 16 (100% 不一致)** | ガードは `+X` と `+Y` を出力するが、幾何空間の分布ズレにより MLP は `CONTINUE` (8) を連発 |

### 12.2 現行方式（連続広域介入） vs 限定補正方式（手設計ガード同一条件での介入）の 15 run アブレーション結果
学習モデルの担当範囲を変える診断比較として、Base（固定）、配置 Candidate（固定）、運搬チェックポイント（固定）の下で、5 つの構成を比較しました（`runs/apc-ablation-scope-results.json`）。

| Seed | 構成 | 介入範囲モード | 最終成否 | Hold 20維持 | 総step | 最小距離 | 内訳 (Base/Trans/Place) | 考察・効果 |
|---|---|---|---|---|---|---|---|---|
| **3007** | 手設計ガード | 手設計条件のみ | **成功** | **達成** | 804 | 0.0154 m | 776 / 6 / 22 | 比較基準 |
| | mlp_continuous | 区間全体 (現行) | **成功** | **達成** | 804 | 0.0154 m | 706 / 76 / 22 | 現行方式でも成功 |
| | **mlp_guarded** | **限定補正 (新)** | **成功** | **達成** | **804** | **0.0154 m** | **776 / 6 / 22** | **手設計ガードとステップ・内訳が完全一致** |
| | cart_continuous | 区間全体 (現行) | **成功** | **達成** | 927 | 0.0130 m | 723 / 180 / 24 | 現行方式で 927 steps |
| | **cart_guarded** | **限定補正 (新)** | **成功** | **達成** | **804** | **0.0154 m** | **776 / 6 / 22** | **限定補正化で 804 steps へ短縮し完全一致** |
| **3011** | 手設計ガード | 手設計条件のみ | **成功** | 失敗 | 889 | 0.0097 m | 866 / 2 / 21 | ガード介入2回で成功 |
| | mlp_continuous | 区間全体 (現行) | 失敗 | 失敗 | 1200 | 0.1187 m | 654 / 546 / 0 | 広域介入で 546 steps 停滞 |
| | **mlp_guarded** | **限定補正 (新)** | 失敗 | 失敗 | 1200 | **0.0393 m** | 1193 / 7 / 0 | **過剰介入が排除され 3.93 cm まで急接近（-Y不足で停滞）** |
| | cart_continuous | 区間全体 (現行) | 失敗 | 失敗 | 1200 | 0.1025 m | 641 / 559 / 0 | 広域介入で 559 steps 停滞 |
| | **cart_guarded** | **限定補正 (新)** | 失敗 | 失敗 | 1200 | **0.0393 m** | 1192 / 8 / 0 | 同上 |
| **3012** | 手設計ガード | 手設計条件のみ | **成功** | **達成** | 745 | 0.0086 m | 705 / 16 / 24 | 比較基準 |
| | mlp_continuous | 区間全体 (現行) | 失敗 | 失敗 | 1200 | 0.0455 m | 1058 / 142 / 0 | 広域介入で 1200 steps 失敗 |
| | **mlp_guarded** | **限定補正 (新)** | **成功** | 失敗 | **755** | **0.0148 m** | **682 / 52 / 21** | **限定補正にしただけで 755 steps で最終成功へ劇的改善！** |
| | cart_continuous | 区間全体 (現行) | 失敗 | 失敗 | 1200 | 0.1280 m | 826 / 374 / 0 | 広域介入で 12.8 cm 停滞 |
| | cart_guarded | 限定補正 (新) | 失敗 | 失敗 | 1200 | 0.0690 m | 1175 / 25 / 0 | 最小距離は 6.9 cm へ改善するも閾値未達 |

#### アブレーション実験から得られた重要知見
1. **早すぎる・広すぎる介入の弊害の実証（seed 3012）**:
   - 連続広域介入（`mlp_continuous`）では 1200 steps で失敗していた Temporary MLP が、**手設計ガードと同一の限定介入条件（`mlp_guarded`）にしただけで 755 steps で配置パッチ到達・最終成功を達成**しました。
   - これは、学習モデルの能力不足だけでなく、「Base に任せるべき正常区間まで学習モデルが奪っていたこと」が失敗の主因であったことを実験的に明確に証明しています。
2. **介入条件限定と教師データ不足の切り分け（seed 3011）**:
   - seed 3011 では限定補正によって不要な介入が 546 steps から 7 steps へ削減され、最小距離は 11.8 cm から 3.93 cm まで大幅に改善しました。
   - 一方で、ガード発動局面で必要な `-Y`（`primitive 3`）が学習データに存在しないため、最後の引き継ぎ（1.6 cm）には届きませんでした。
   - これにより、「介入条件の限定」だけでは解決できない真の教師データ不足（未学習方向 `-Y`）を正確に分離して特定できました。

### 12.3 更新後モデルによる回帰確認
更新版モデル（`runs/apc-transit-temp-train-20260925-b` および `runs/apc-transit-cand-cart-20260925-b`）を用い、過去に成功していた条件の保持を直接検証しました。

| タスク | Seed | 評価目的 | 構成 | 最終成否 | Hold 20維持 | 総step | 実測内訳・備考 |
|---|---|---|---|---|---|---|---|
| TruePlaceFar | 3001 | 元の成功配置の保持 | 手設計ガード | **成功** | **達成** | 859 | 837 / 0 / 22 (Transit非介入で成功完全維持) |
| | | | 更新 Temporary MLP | **成功** | **達成** | 859 | 837 / 0 / 22 (同上) |
| | | | 更新 Candidate CART | **成功** | **達成** | 859 | 837 / 0 / 22 (同上) |
| TruePlaceFar | 3002 | 開発配置の保持 | 手設計ガード | **成功** | **達成** | 789 | 762 / 3 / 24 |
| | | | 更新 Temporary MLP | **成功** | **達成** | 789 | 669 / 96 / 24 |
| | | | 更新 Candidate CART | **成功** | 失敗 | 861 | 690 / 149 / 22 (最終成功達成) |
| PickCubeFar | 3201 | 空中Pick能力の非干渉保持 | 更新 Temporary MLP | **成功** | **達成** | **745** | 目標高さ分離によりパッチ非干渉、Pick能力 100% 保持 |
| | | | 更新 Candidate CART | **成功** | **達成** | **745** | 同上 |

- **結論**: 更新後モデルにおいても、元配置 seed 3001 および PickCubeFar seed 3201 の成功（非干渉）が直接実証されました。
