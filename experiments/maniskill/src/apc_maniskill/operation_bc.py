...
def load_demonstrations(path: Path, *, successful_only=False):
    ...
    if successful_only and scripted:
        rows = [row for row in rows if row["success_ever"]]
        if not rows:
    ...
    for row in rows:
        ...
        with np.load(trajectory, allow_pickle=False) as data:
            ...
            ys.append(labels[:, 0:11].astype(np.float32))
...
def train(demo_run: Path, output: Path, *, updates=1000, seed=0, hidden_width=64,
          successful_only=False, extra_runs=()) -> Path:
    ...
    try:
        ...
        datasets = [load_demonstrations(path, successful_only=successful_only)
