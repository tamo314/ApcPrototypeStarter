"""Tests for the frozen high-capacity operator upper bound benchmark (Phase
A.1 diagnostic Task A1-R005E-004).

Fast, small-step tests only -- mirroring `tests/test_oracle_latent_operator_
benchmark.py`/`tests/test_sequence_counterfactual_gate.py`'s convention of
exercising the wiring (pure helpers, group construction, config validation,
operator forward shape, per-operation benchmark, multi-seed aggregation)
with a tiny model and a handful of training steps. The benchmark's own
scientific claim (5 seeds, the full 30000-step core / 8000-step operator
budget) is run via `scripts/frozen_high_capacity_operator_benchmark.py`, not
asserted here.
"""
...
def _tiny_config(**overrides: object) -> FrozenHighCapacityOperatorConfig:
    base = FrozenHighCapacityOperatorConfig(
        seed=0,
        vocab_size=6,
...
def test_generated_groups_are_deterministic_given_same_seed_step_split() -> None:
    a = generate_high_capacity_operator_counterfactual_groups(
        1, 5, operation="COUNT", step=2, split="train", vocab_size=6, sequence_length_range=(4, 6)
    )
    b = generate_high_capacity_operator_counterfactual_groups(
        1, 5, operation="COUNT", step=2, split="train", vocab_size=6, sequence_length_range=(4, 6)
    )
...
def test_config_from_dict_round_trips_defaults_and_overrides() -> None:
    ...
    config = frozen_high_capacity_operator_config_from_dict(raw)
    assert config.seed == 2
    assert config.operation_names == ("SHIFT",)
    assert config.model["d_model"] == 16
    assert config.core_train.steps == 3
    assert config.d_operator == 16
    assert config.operator_train.steps == 3

    defaulted = frozen_high_capacity_operator_config_from_dict({})
    assert defaulted == FrozenHighCapacityOperatorConfig()


def test_default_operation_names_are_the_four_parameterized_operations() -> None:
    assert set(FrozenHighCapacityOperatorConfig().operation_names) == {
        "SHIFT",
        "SELECT",
        "COUNT",
        "BIND",
...
# --- HighCapacityOperator forward shape/masking -----------------------------


def _tiny_operator(operation: str) -> HighCapacityOperator:
    return HighCapacityOperator(
        operation,
        d_model=8,
        d_operator=12,
        n_layers=2,
...
def test_run_frozen_high_capacity_operator_benchmark_report_shape() -> None:
    config = _tiny_config()
    report = run_frozen_high_capacity_operator_benchmark(config)
...
def test_run_frozen_high_capacity_operator_benchmark_writes_metrics(tmp_path: object) -> None:
    from pathlib import Path

    metrics_dir = Path(str(tmp_path)) / "seed_0"
    config = _tiny_config(operation_names=("SHIFT",))
    run_frozen_high_capacity_operator_benchmark(config, metrics_dir=metrics_dir)
...
def test_multi_seed_aggregation_matches_manual_formula() -> None:
    ...
    seeds = (0, 1)
    result = run_frozen_high_capacity_operator_benchmark_multi_seed(base_config, seeds=seeds)
