"""Tests for the counterfactual BIND gate (Phase A.1 Post-Correction Task
A1-R005D-007).
...
is run via `scripts/bind_counterfactual_gate.py`, not asserted here.
"""
...
from apc.evaluation.bind_counterfactual_gate import (
    CORRECT_THRESHOLD,
    DEFAULT_GROUP_SIZE,
    DEFAULT_SEEDS,
...
def _tiny_config(**overrides: object) -> BindCounterfactualGateConfig:
    base = BindCounterfactualGateConfig(
        seed=0,
        vocab_size=6,
...
def test_valid_bind_lengths_keeps_only_even_lengths_at_least_two() -> None:
    assert _valid_bind_lengths((3, 8)) == (4, 6, 8)


def test_valid_bind_lengths_excludes_below_two() -> None:
    assert _valid_bind_lengths((0, 3)) == (2,)


def test_valid_bind_lengths_empty_when_range_has_no_even_length() -> None:
    assert _valid_bind_lengths((1, 1)) == ()


# --- generate_bind_counterfactual_groups / BindCounterfactualGroup --------


def test_generated_groups_have_pairwise_distinct_outputs() -> None:
    groups = generate_bind_counterfactual_groups(0, 50, step=0, split="train")
    for group in groups:
...
def test_generated_groups_share_the_same_content() -> None:
    groups = generate_bind_counterfactual_groups(0, 20, step=0, split="train")
    for group in groups:
        for example in group.examples:
            assert example.input_tokens == group.input_tokens


def test_generated_groups_have_even_length_content() -> None:
    groups = generate_bind_counterfactual_groups(0, 50, step=0, split="train")
    for group in groups:
        assert len(group.input_tokens) % 2 == 0
        assert len(group.input_tokens) >= 2


def test_generated_groups_query_keys_are_present_in_content() -> None:
    groups = generate_bind_counterfactual_groups(0, 50, step=0, split="train")
    for group in groups:
        present_keys = set(group.input_tokens[0::2])
        assert set(group.query_keys) <= present_keys


def test_generated_groups_meet_minimum_size() -> None:
    groups = generate_bind_counterfactual_groups(0, 200, step=0, split="train")
...
def test_generate_groups_is_deterministic_given_the_same_arguments() -> None:
    first = generate_bind_counterfactual_groups(0, 10, step=3, split="train")
    second = generate_bind_counterfactual_groups(0, 10, step=3, split="train")
    assert [g.input_tokens for g in first] == [g.input_tokens for g in second]
    assert [g.query_keys for g in first] == [g.query_keys for g in second]


def test_generate_groups_differ_by_step() -> None:
    first = generate_bind_counterfactual_groups(0, 10, step=0, split="train")
    second = generate_bind_counterfactual_groups(0, 10, step=1, split="train")
    assert [g.input_tokens for g in first] != [g.input_tokens for g in second]


def test_generate_groups_rejects_invalid_arguments() -> None:
    with pytest.raises(ValueError, match="n_groups"):
        generate_bind_counterfactual_groups(0, 0, step=0, split="train")
    with pytest.raises(ValueError, match="step"):
        generate_bind_counterfactual_groups(0, 1, step=-1, split="train")
    with pytest.raises(ValueError, match="group_size"):
        generate_bind_counterfactual_groups(0, 1, step=0, split="train", group_size=1)


def test_generate_groups_rejects_a_range_with_no_valid_bind_length() -> None:
    with pytest.raises(ValueError, match="even length"):
        generate_bind_counterfactual_groups(
            0, 1, step=0, split="train", sequence_length_range=(3, 3)
        )


def test_group_rejects_mismatched_examples_and_query_keys() -> None:
    groups = generate_bind_counterfactual_groups(0, 1, step=0, split="train")
    group = groups[0]
    with pytest.raises(ValueError, match="len\\(examples\\) == len\\(query_keys\\)"):
        BindCounterfactualGroup(
            input_tokens=group.input_tokens,
            examples=group.examples,
            query_keys=group.query_keys[:-1],
        )
...
def test_group_rejects_non_distinct_outputs() -> None:
    ...
    with pytest.raises(ValueError, match="pairwise distinct outputs"):
        BindCounterfactualGroup(
            ...
            query_keys=(group.query_keys[0], group.query_keys[0]),
        )


def test_wrong_key_index_points_to_a_different_member() -> None:
    groups = generate_bind_counterfactual_groups(0, 20, step=0, split="train")
    for group in groups:
        for index in range(len(group.examples)):
            partner = group.wrong_key_index(index)
...
def test_flatten_groups_preserves_total_example_count() -> None:
    groups = generate_bind_counterfactual_groups(0, 10, step=0, split="train")
...
def test_flatten_groups_wrong_call_is_always_effectful() -> None:
    groups = generate_bind_counterfactual_groups(0, 30, step=0, split="train")
...
def test_flatten_groups_wrong_call_is_a_well_formed_bind_call() -> None:
    groups = generate_bind_counterfactual_groups(0, 5, step=0, split="train")
    examples, wrong_call_by_id, _ = _flatten_groups(groups)
    for example in examples:
        call = wrong_call_by_id[id(example)]
        assert call.operation == "BIND"
        assert "query_key" in call.arguments


# --- BindCounterfactualGateConfig validation ----------------------------------
...
def test_config_rejects_sequence_length_range_with_no_even_length() -> None:
    with pytest.raises(ValueError, match="even length"):
        _tiny_config(sequence_length_range=(3, 3))
...
def test_config_from_dict_fills_in_defaults() -> None:
    config = bind_counterfactual_gate_config_from_dict({"seed": 3})
    assert config.seed == 3
    assert config.group_size == DEFAULT_GROUP_SIZE
    assert config.primitive_rank == BindCounterfactualGateConfig().primitive_rank
...
def test_config_from_dict_parses_nested_train_configs() -> None:
    ...
    config = bind_counterfactual_gate_config_from_dict(raw)
...
def test_build_primitive_bank_registers_exactly_one_bind_primitive() -> None:
    bank = _build_primitive_bank(d_model=16, rank=4, vocab_size=6, max_sequence_length=8, arg_dim=6)
    assert len(bank) == 1
    primitive = bank.get(operation_id("BIND"))
...
def test_build_primitive_bank_primitive_is_trainable() -> None:
    bank = _build_primitive_bank(d_model=16, rank=4, vocab_size=6, max_sequence_length=8, arg_dim=6)
    assert not bank.get(operation_id("BIND")).is_frozen()
...
def test_build_primitive_bank_size_does_not_grow_with_argument_values() -> None:
    ...
    h = torch.randn(1, 16)
    bind = bank.get(operation_id("BIND"))
    for query_key in range(5):
        bind(h, argument_values=[query_key])
...
def test_run_returns_report_with_expected_fields() -> None:
    report = run_bind_counterfactual_gate(_tiny_config())
    assert isinstance(report, BindCounterfactualGateReport)
...
def test_run_is_deterministic_given_the_same_seed() -> None:
    first = run_bind_counterfactual_gate(_tiny_config())
    second = run_bind_counterfactual_gate(_tiny_config())
...
def test_multi_seed_runs_one_report_per_seed_and_writes_run_dir(tmp_path: Path) -> None:
    seeds = (0, 1)
    result = run_bind_counterfactual_gate_multi_seed(_tiny_config(), seeds=seeds, run_dir=tmp_path)
...
def test_multi_seed_overrides_base_config_seed_per_entry() -> None:
    result = run_bind_counterfactual_gate_multi_seed(_tiny_config(), seeds=(0, 1, 2))
    assert [report.config.seed for report in result.per_seed] == [0, 1, 2]


def test_multi_seed_mean_causal_gap_matches_mean_arms() -> None:
    result = run_bind_counterfactual_gate_multi_seed(_tiny_config(), seeds=(0, 1))
...
def test_multi_seed_family_count_passed_reflects_bank_size() -> None:
    result = run_bind_counterfactual_gate_multi_seed(_tiny_config(), seeds=(0,))
...
def test_multi_seed_meets_seed_policy_reflects_seed_count() -> None:
    short = run_bind_counterfactual_gate_multi_seed(_tiny_config(), seeds=(0, 1))
    assert short.meets_seed_policy is False
    full = run_bind_counterfactual_gate_multi_seed(
...
def test_multi_seed_report_to_dict_round_trips_through_json(tmp_path: Path) -> None:
    result = run_bind_counterfactual_gate_multi_seed(
...
def test_thresholds_match_a1_r005d_007() -> None:
