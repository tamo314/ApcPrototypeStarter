#!/usr/bin/env python
"""CLI entry point for Recurrence & Reuse Benchmark (Task A1-B007 / Milestone B-M7).

Demonstrates that when previously consolidated operations reappear in a lifelong task stream,
the system retrieves and executes the installed primitive immediately without allocating
plastic capacity or requiring retraining.

Usage:
    python scripts/recurrence_benchmark.py \
        --config configs/phase_a1_recurrence_benchmark.yaml \
        --run-dir runs/phase_a1_recurrence_benchmark
"""
...
from apc.evaluation.recurrence_benchmark import (
    ...
    run_recurrence_benchmark_multi_seed,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Path to phase_a1_recurrence_benchmark.yaml",
    )
    parser.add_argument(
        "--run-dir",
        required=True,
    ...
    base_config = recurrence_config_from_dict(raw_config)
    ...
    def config_factory(s: int) -> RecurrenceBenchmarkConfig:
        return dataclasses.replace(base_config, seed=s)

    result = run_recurrence_benchmark_multi_seed(
    ...
    summary = {
        ...
        "mean_immediate_exact_match": result.mean_immediate_exact_match,
    ...
    print(f"\nTask A1-B007 Recurrence & Reuse Benchmark: {verdict}")
