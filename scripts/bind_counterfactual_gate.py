#!/usr/bin/env python
"""CLI entry point for the Counterfactual BIND gate (Phase A.1
Post-Correction Task A1-R005D-007).

Per seed: pretrains and freezes a task-blind, BIND-only Stable Core
...
`BIND` primitive on counterfactual groups (`apc.evaluation.
bind_counterfactual_gate.generate_bind_counterfactual_groups` -- the same
content paired with several distinct keys whose outputs are pairwise
...
Usage:
    python scripts/bind_counterfactual_gate.py \\
        --config configs/phase_a1_bind_counterfactual_gate.yaml \\
        --run-dir runs/phase_a1_bind_counterfactual_gate
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Path to a phase_a1_bind_counterfactual_gate.yaml-shaped config",
    )
    parser.add_argument(
    ...
    base_config = bind_counterfactual_gate_config_from_dict(raw_config)
    ...
    print(
        f"A1-R005D-007 Counterfactual BIND gate: {verdict}{seed_note}{stop_gate_note}"
    )
