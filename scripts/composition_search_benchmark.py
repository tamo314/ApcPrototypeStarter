#!/usr/bin/env python
"""CLI entry point for Composition Search Baseline Benchmark (Phase A.1 Post-Diagnostic
Task A1-B004, Milestone B-M4).
...
Usage:
    python scripts/composition_search_benchmark.py \
        --config configs/phase_a1_composition_search_benchmark.yaml \
        --run-dir runs/phase_a1_composition_search_benchmark
"""
...
from apc.evaluation.composition_search_benchmark import (
    ...
    composition_search_config_from_dict,
    run_composition_search_benchmark_multi_seed,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Path to phase_a1_composition_search_benchmark.yaml",
    )
    parser.add_argument(
        "--run-dir",
        required=True,
    ...
    base_config = composition_search_config_from_dict(raw_config)
    ...
    def config_factory(s: int) -> CompositionSearchBenchmarkConfig:
        return dataclasses.replace(base_config, seed=s)

    result = run_composition_search_benchmark_multi_seed(
    ...
    summary = {
        ...
        "mean_overall_recovered_exact_match": result.mean_overall_recovered_exact_match,
        "mean_overall_functional_agreement": result.mean_overall_functional_agreement,
    ...
    print(f"Task A1-B004 Composition Search Baseline Benchmark: {verdict}")
