"""Run Phase B Task B-C005R2: Functional Adequacy Estimator Repair Benchmark."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Phase B Task B-C005R2 Functional Adequacy Estimator Repair"
    )
    parser.add_argument(
        "--config", type=Path, default=Path("configs/phase_b_b2_adequacy_repair.yaml")
    )
    ...
    if args.policies is not None:
        config_dict["policies"] = tuple(args.policies)
    elif "policies" in config_dict:
        config_dict["policies"] = tuple(config_dict["policies"])
    ...
    config = AdequacyRepairConfig(**config_dict)
