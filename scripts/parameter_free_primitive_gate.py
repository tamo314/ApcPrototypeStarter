#!/usr/bin/env python
"""CLI entry point for the Parameter-free oracle primitive benchmark (Phase
A.1 Post-Correction Task A1-R003, STOP GATE, H2b).
...
restricted to `apc.environments.operations.DETERMINISTIC_OPERATION_NAMES`),
trains one oracle-routed primitive per operation on top of it, then evaluates
the causal ablation matrix's Correct/Wrong/None arms on a large unseen batch.
...
Usage:
    python scripts/parameter_free_primitive_gate.py \\
        --config configs/phase_a1_parameter_free_primitive_gate.yaml \\
        --run-dir runs/phase_a1_parameter_free_primitive_gate
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Path to a phase_a1_parameter_free_primitive_gate.yaml-shaped config",
    )
    parser.add_argument(
    ...
    base_config = parameter_free_primitive_gate_config_from_dict(raw_config)
    ...
    summary = {
        ...
        "per_seed": {
            str(report.config.seed): {
                ...
                "wrong_exact_match": report.wrong_exact_match,
    ...
    print(
        f"A1-R003 Parameter-free oracle primitive benchmark: {verdict}{seed_note}{stop_gate_note}"
    )
