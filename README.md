...
- **NRQ-003 (`BLOCKED_BY_MODEL_ADEQUACY`)**: [prerequisite audit and model adequacy gate](docs/research/EXACT_DEPTH3_COMPOSITION_SEARCH_AUDIT_NRQ003.md) for the exact-depth-3 irreducible composition search benchmark. Audited frozen core and primitive-bank bundle provenance across 5 seeds and evaluated Task A1-B004 depth-2 positive controls across 6 canonical recipes. Found broken bundle provenance: seeds 1-4 possess token_emb shape [36, 192] incompatible with current vocab_size=44 (`RuntimeError: size mismatch`); seed 0 possesses shape [44, 192] but exhibits latent space de-synchronization with primitive_bank.pt, causing depth-2 positive controls to collapse to 11.8% mean oracle exact match and 18.3% recovered exact match (0/6 passed). Following the mandatory prerequisite contract, execution and interpretation of depth-3 search are strictly barred, recording NRQ-003 as `BLOCKED_BY_MODEL_ADEQUACY` ([ADR-0163](docs/DECISIONS_PHASE_C.md#adr-0163-nrq-003-exact-depth-3-composition-search-prerequisite-check-blocked-by-bundle-provenance-and-model-inadequacy-blocked_by_model_adequacy)). No task is queued.
- **NRQ-004 (`STOP_NRQ003_BLOCKED`)**: [frozen bundle compatibility reconstruction and depth-2 control reproduction](docs/research/FROZEN_BUNDLE_COMPATIBILITY_RECONSTRUCTION_NRQ004.md). Reconstructed native Phase A.1 36-token vocabulary schema into non-destructive namespace `runs/nrq004_reconstructed_bundles/`. Seeds 1-4 reproduced near-ceiling depth-2 controls (mean recovered EM = 99.50%, functional agreement = 99.96%, passing 6/6), definitively refuting intrinsic model inadequacy. Seed 0 failure causally isolated to irreversible bundle loss (checkpoint overwritten on 2026-09-13). Following fail-closed stop criteria, NRQ-003 resumption is barred and execution halts without training or depth-3 search ([ADR-0164](docs/DECISIONS_PHASE_C.md#adr-0164-nrq-004-frozen-bundle-compatibility-reconstruction--depth-2-control-reproduction-fails-on-bundle-loss-stop_nrq003_blocked)). No task is queued.
- **NRQ-005 (`EMPIRICALLY_SUPPORT_CLOSURE_DEPTH_LE_3`)**: [missing-artifact-robust exact-depth-3 irreducible composition benchmark](docs/research/EXACT_DEPTH3_COMPOSITION_BENCHMARK_NRQ005.md). Independent replacement protocol using intact seeds 1–4 from `runs/nrq004_reconstructed_bundles/` with Seed 0 excluded for historical artifact loss. Verified exact-depth-3 irreducibility via symbolic probe (0/72 depth <= 2 matches); confirmed oracle capability ceiling (mean Oracle EM = 99.05% >= 0.85); evaluated exhaustive lawful search over all 584 candidates (99.04% mean test EM, >= 0.95 across all tested bundles and data seeds). Proved that exact-depth-3 compositions remain within the tractable reach of lawful deterministic search, empirically supporting ADR-0162's Bounded-Resource Corollary restricted to depth <= 3 ([ADR-0165](docs/DECISIONS_PHASE_C.md#adr-0165-nrq-005-missing-artifact-robust-exact-depth-3-irreducible-composition-benchmark-supports-depth--3-closure-empirically_support_closure_depth_le_3)). No task is queued.
- **NRQ-006 (`ADR0165_QUALIFIED_BY_FULL_REGISTRY_AUDIT`)**: [argument-closed deterministic full-registry depth-3 closure audit](docs/research/ARGUMENT_CLOSED_DEPTH3_CLOSURE_AUDIT_NRQ006.md). Audited all 512 candidate exact-depth-3 recipes over the canonical 8-primitive registry against all 72 depth <= 2 candidates under a pre-fixed finite lawful argument-transform grammar. Enforced inter-process determinism via SHA-256 seed derivation, verified bitwise identical across independent processes (dataset manifest SHA-256 `eeee5ac21698a6567e697991f0f97616c0c7963c72635bf922621803f9284d45`, metric deltas = 0.00e+00). Partitioned recipes into 42 invalid, 370 reducible, and 100 irreducible recipes spanning 60 canonical equivalence classes. Confirmed depth <= 3 closure for length-adequate classes. Discovered sub-curriculum intermediate length contraction under SELECT ($L < 6$); formally qualified ADR-0165's unconditional closure claim ([ADR-0166](docs/DECISIONS_PHASE_C.md#adr-0166-nrq-006-argument-closed-deterministic-full-registry-depth-3-audit-qualifies-adr-0165-closure-adr0165_qualified_by_full_registry_audit)). Re-aggregated and formally amended by Task NRQ-007 ([ADR-0167](docs/DECISIONS_PHASE_C.md#adr-0167-nrq-007-stepwise-causal-attribution-disentangles-depth-3-failure-mechanisms-adr0166_qualified_by_stepwise_causal_attribution)). No task is queued.
- **NRQ-007 (`ADR0166_QUALIFIED_BY_STEPWISE_CAUSAL_ATTRIBUTION`)**: [stepwise causal attribution of NRQ-006 composition failures](docs/research/STEPWISE_CAUSAL_ATTRIBUTION_NRQ007.md). Re-aggregated all 1200 benchmark cells from NRQ-006, reconciling historical counts and enforcing the non-averaging all-cell gate (19/60 classes pass all 20 cells, 558/1200 cells pass both thresholds). Executed 3-arm stepwise causal intervention across all 35 failure recipes ($700$ cells) comparing (A) continuous execution, (B) diagnostic reset with ground truth intermediate re-encoding, (C) standalone length-matched controls, and parameterized controls. Programmatically attributed failure mechanisms to short-sequence capacity deficit in `SORT` on $L < 6$ (7 classes, 330 cells), argument handling on $L=1$ intermediates (4 classes, 250 cells), and cross-bundle parameter divergence (24 classes). Formally amended ADR-0166 to `QUALIFIED` ([ADR-0167](docs/DECISIONS_PHASE_C.md#adr-0167-nrq-007-stepwise-causal-attribution-disentangles-depth-3-failure-mechanisms-adr0166_qualified_by_stepwise_causal_attribution)) and reaffirmed `PROGRAM_LINE_CLOSURE_CONFIRMED`. No task is queued.
- **NRQ-008 (`FINITE_SAMPLE_SUPPORT_MISSELECTION`)**: [replication and support-budget sensitivity of sole APC-over-exhaustive cell](docs/research/REPLICATION_AND_SUPPORT_BUDGET_SENSITIVITY_NRQ008.md). Evaluated solitary cell where APC appeared to outperform exhaustive search (`NEGATE->REVERSE->SHIFT` on Bundle 4, Seed 101) across intact bundles 1–4, 20 unused data seeds (201–220), support budgets $N \in \{32, 64, 128, 256\}$, and 1024 disjoint held-out inputs per cell (320 cells, 327,680 total eval queries). Demonstrated across two independent processes ($\Delta = 0.00\times 10^0$) that element negation and reversal commute, exhaustive search achieves parity with oracle execution across all budgets ($\Delta \le 0.0010$, $\Delta = 0.0007$ at $N=256$), zero bundles satisfy the falsification criteria, Bundle 4's lower performance ($0.9129$ vs $0.9147$) is an intrinsic substrate capacity limit rather than a search failure, and conclusively closed the solitary anomaly as a finite-sample support mis-selection artifact under small sample budget ([ADR-0168](docs/DECISIONS_PHASE_C.md#adr-0168-nrq-008-replication-and-support-budget-sensitivity-audit-closes-solitary-anomaly-cell-as-finite-sample-support-mis-selection-finite_sample_support_misselection)). Reaffirmed `PROGRAM_LINE_CLOSURE_CONFIRMED`. No task is queued.
